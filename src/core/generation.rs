...
use rand::{Rng, SeedableRng}; // 乱数生成のために追加！
use noise::{OpenSimplex, NoiseFn}; // ✨ ノイズ生成のために追加！
use rand::rngs::StdRng;      // シード値から初期化できる乱数生成器だよ
...
/// * `seed` - 地形生成に使用するシード値。
pub fn generate_initial_world(world: &mut World, seed: u64) {
  // シード値から乱数生成器を初期化するよ！これで同じシードなら同じ乱数列が作られるんだ。
  let mut rng = StdRng::seed_from_u64(seed);

  // --- 地表生成用のノイズ関数 ---
  // ワールドのシードとは別のシードを使うことで、地表のパターンに多様性を持たせるよ。
  let surface_noise_seed = seed.wrapping_add(1); // シードをずらす
  ...
  let primary_cave_noise_seed = seed.wrapping_add(2);
  ...
  let region_cave_noise_seed = seed.wrapping_add(3);
  ...
  let temperature_noise_seed = seed.wrapping_add(4);
  ...
  let precipitation_noise_seed = seed.wrapping_add(5);
  ...
  for x in 0..WIDTH {
    ...
    let surface_offset_noise = surface_noise.get([
        ...
        (seed % 1000) as f64 * 0.05 // X座標だけでなく、シード値からも少し影響を受けるようにして、ワールドごとのパターンを増やすよ。
    ]);
    ...
    let temp_val = temperature_noise.get([x as f64 * temperature_scale, (seed % 100) as f64 * 0.02]);
    let precip_val = precipitation_noise.get([x as f64 * precipitation_scale, (seed % 100 + 50) as f64 * 0.02]); // シードを少しずらす
    ...
    match current_biome {
      Biome::Forest => {
        rock_layer_start_y = (current_dirt_surface_y as i32 + rng.gen_range(rock_layer_start_depth_offset_min..rock_layer_start_depth_offset_max)) as usize;
        // 森林の洞窟は標準的
      }
      Biome::Desert => {
        rock_layer_start_y = (current_dirt_surface_y as i32 + rng.gen_range(rock_layer_start_depth_offset_min - 5 .. rock_layer_start_depth_offset_max - 5).max(10)) as usize; // 砂漠は岩盤が少し浅めかも
      ...
      Biome::Snowland => {
        rock_layer_start_y = (current_dirt_surface_y as i32 + rng.gen_range(rock_layer_start_depth_offset_min + 5 .. rock_layer_start_depth_offset_max + 5)) as usize; // 雪原は岩盤が少し深めかも
    ...
    for y in 0..HEIGHT {
      if y < current_dirt_surface_y {
        ...
        let terrain_roll = rng.random::<f64>(); // 0.0 から 1.0 の乱数を生成。
        ...
        if y >= rock_layer_start_y { // 岩盤層
          ...
          let deep_rock_zone_start_offset = (rng.gen_range(25..50)) as f64; // 「深い岩盤層」が始まるオフセット
          let deepest_rock_zone_start_offset = (rng.gen_range(70..100)) as f64; // 「最深岩盤層」が始まるオフセット
          ...
          if terrain_roll < forest_weight * 0.8 + desert_weight * 0.2 + snow_weight * 0.1 { // Dirtが出やすい範囲 (Forestの影響大)
              determined_terrain = Terrain::Dirt;
              if snow_weight > 0.3 && rng.gen_bool(snow_weight * 0.5) { determined_overlay = Overlay::Ice; } // 雪原の影響が強ければ凍った土に
          } else if terrain_roll < forest_weight * 0.9 + desert_weight * 0.7 + snow_weight * 0.2 { // Sandが出やすい範囲 (Desertの影響大)
              determined_terrain = Terrain::Sand;
          } else if terrain_roll < forest_weight * 0.95 + desert_weight * 0.8 + snow_weight * 0.6 { // HardIceが出やすい範囲 (Snowlandの影響大)
                determined_terrain = Terrain::HardIce;
          } else { // それ以外はRockや鉱石など (バイオームの影響は小さめ)
                determined_terrain = Terrain::Rock; // たまに岩が混じる
                if rng.gen_bool(0.05) { determined_terrain = Terrain::Coal; } // 鉱石も少し
          }
        }
        ...
        } else if y > current_dirt_surface_y && y < current_dirt_surface_y + rng.gen_range(3..6) { // 地表から数ブロック下の調整
          match current_biome { // このブロックは、より詳細な表層の調整に使えそう。雪原の表層も雪や氷っぽくする？
            Biome::Desert => { // 砂漠なら、表層も砂で置き換える確率を上げる
              if rng.gen_bool(0.75) { determined_terrain = Terrain::Sand; }
            }
            Biome::Snowland => { // 雪原なら、表層が土の場合、一部を雪や氷（まだないけど）っぽくしても面白いかも
              if determined_terrain == Terrain::Dirt && rng.gen_bool(0.3) {
        ...
        if determined_terrain.allows_fluid_passthrough() && determined_overlay == Overlay::Air && y > current_dirt_surface_y + rng.gen_range(3..8) {
            if rng.gen_bool(0.0015) { // 0.15% の確率で水源を設置！
        ...
        let min_cave_depth_from_surface = rng.gen_range(8..15); // 地表から最低でもこの深さから洞窟が始まるようにする。
        ...
        if y >= cave_start_y {
          ...
          if region_noise_val > current_region_cave_influence_threshold { // バイオームごとの閾値を使用
            ...
            if primary_noise_val > current_cave_threshold_low && primary_noise_val < current_cave_threshold_high { // バイオームごとの閾値を使用
              world.set_terrain(x, y, Terrain::Empty); // 地形を空にする！
              if rng.gen_bool(0.008) { // 洞窟内にごく稀に水源を設置。
                world.set_overlay(x, y, Overlay::Water);
              } else {
  ...
  for x in 0..WIDTH {
    ...
    let surface_offset_noise = surface_noise.get([x as f64 * surface_scale, (seed % 1000) as f64 * 0.05]);
    let surface_y_fluctuation = surface_offset_noise * surface_amplitude;
    let current_surface_y = (base_surface_y_level + surface_y_fluctuation).round().max(15.0).min((HEIGHT - 40) as f64) as usize;

    // バイオームを再決定
    let temp_val = temperature_noise.get([x as f64 * temperature_scale, (seed % 100) as f64 * 0.02]);
    let precip_val = precipitation_noise.get([x as f64 * precipitation_scale, (seed % 100 + 50) as f64 * 0.02]);
            ...
            if surface_tile.terrain == Terrain::Dirt {
              ...
              if let Some(overlay_above) = world.get_overlay(x, y_on_surface) {
                ...
                if *overlay_above == Overlay::Air {
                  if rng.gen_bool(0.7) { world.set_overlay(x, y_on_surface, Overlay::Grass); } // 高確率で草
                  if rng.gen_bool(0.15) { world.set_overlay(x, y_on_surface, Overlay::Tree); } // たまに木
                }
              }
            }
          }
        ...
        Biome::Desert => {
          ...
          if let Some(surface_tile) = world.get(x, current_surface_y) {
            ...
            if surface_tile.terrain == Terrain::Sand {
              ...
              if world.get_overlay(x, y_on_surface) == Some(&Overlay::Air) && rng.gen_bool(0.03) {
          ...
          if let Some(surface_tile) = world.get(x, current_surface_y) {
            ...
            if surface_tile.terrain == Terrain::Dirt {
              ...
              if world.get_overlay(x, y_on_surface) == Some(&Overlay::Air) {
                if rng.gen_bool(0.4) { world.set_overlay(x, y_on_surface, Overlay::Snow); } // 地表の雪ブロックの上にさらに積もった雪
                if rng.gen_bool(0.02) { world.set_overlay(x, y_on_surface, Overlay::Tree); } // 雪原にも稀に針葉樹
              }
            }
          }
        }
